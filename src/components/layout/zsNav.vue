    ...
    <div @mouseleave="onMouseLeave($event)" class="zs-side-scroll">
      <ul class="zs-nav">
        <template v-for="(item,index) in dataList">
          <li :class="{'zs-nav-itemed':item.isCur == true}" @mouseenter="onMouseEnter('1',$event)"
              class="zs-nav-item" v-if="item.children&&item.children.length>0">
            <a @click="onItemClick(item,index)" href="javascript:void(0);">{{item.name}}
              <span class="zs-nav-more"></span>
            </a>
            <dl class="zs-nav-child">
              <dd :class="{'zs-this':activeIndex== index+'-'+childIndex}" v-for="(child,childIndex) in item.children">
                <a @click="onItemChildClick(child)" href="javascript:void(0);">{{child.name}}</a>
              </dd>
            </dl>
          </li>
          <li :class="{'zs-this':activeIndex== index+'-0'}" @mouseenter="onMouseEnter('1',$event)" class="zs-nav-item"
              v-else>
            <a @click="onItemChildClick(item)" href="javascript:void(0);">{{item.name}}
            </a>
          </li>
        </template>
        <li @mouseenter="onMouseEnter('2',$event)" class="zs-nav-item"
            style="height: 30px; text-align: center;color:#fff;background:#fff;"></li>
