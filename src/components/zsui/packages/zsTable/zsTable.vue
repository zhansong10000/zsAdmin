    ...
    <div class="zs-table-body-wrapper">
      <table class="zs-table-body">
        <tr v-for="rowData in dataList">
          <td
            v-for="item in columnModel"
            :width="item.width"
            @click="item.click?item.click(rowData[item.name]):''"
          >{{ item.formatter?item.formatter(rowData[item.name]):rowData[item.name]}}</td>
        </tr>
      </table>
    </div>
    <div class="zs-table-footer-wrapper">
      <div v-if="total!=0" class="zs-table-pagination">
        <span class="zs-table-pagination-total">
          共
          <em>{{toalPage}}</em>页 共
          <em>{{total}}</em>条记录
        </span>
    ...
    calColumnWidth() {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (typeof width == "string" && width.indexOf("%") > 0) {
          width = Math.round(
            (tableWidth * parseInt(width.substring(0, width.length - 1))) / 100
          );
          column.width = width;
        }
